...
1. **FPS ONNX INT8 (dynamic) không đạt real-time trên máy này** (2.7-2.8 FPS, ngưỡng yêu cầu 15 FPS) — khác hẳn báo cáo gốc (16.73 FPS, nghi ngờ đo trên Mac/Apple Silicon).
   ...
   **Phát hiện phụ — User hỏi vì sao baseline "FP32" chỉ 5.98MB (thay vì ~11.5MB lý thuyết):** Đã kiểm chứng: `baseline.pt` có 3,020,988 tham số → FP32 thật phải ~11.52MB, nhưng file chỉ 5.98MB — khớp chính xác với kích thước FP16 (~5.76MB). Nguyên nhân: Ultralytics mặc định gọi `.half()` trước khi ghi checkpoint `best.pt` ra đĩa để tiết kiệm dung lượng, dù khi nạp lại bằng `YOLO(...)` PyTorch tự ép kiểu lên `float32` (`next(model.parameters()).dtype == torch.float32` đã kiểm chứng). Vậy nhãn "Baseline (FP32)" đúng về mặt suy luận (không lượng tử hóa) nhưng số MB hiển thị là kích thước lưu trữ FP16, không phải FP32 thật. Đã thêm chú thích làm rõ vào README.md, docs/bao_cao_tuan_4.md, docs/bao_cao_tuan_5.md (trên nhánh `fix-pruning-fp16`).
